...
var fastAnimationsState;
var autoSafariColor;
var safAutoPickColor;
var fastAnimationsColor;
var characterMovementSpeed;
...
function initAutoSafari() {
    ...
    autoSafariTimer = setInterval(function () {
    ...
    function moveCharacterWithDelay(shortestPath) {
        ...
        function moveNextStep(shortestPath) {
    ...
    function fightSafariPokemon() {
        if (App.game.party.getPokemon(SafariBattle.enemy.id)?.pokerus != GameConstants.Pokerus.Uninfected && App.game.party.getPokemon(SafariBattle.enemy.id)?.evs() < 50) {
            if (SafariBattle.enemy.angry == 0) { SafariBattle.throwRock() }
            else { SafariBattle.throwBall(); }
        }
    ...
    function createHTML() {
        ...
        autoSafariBtn.addEventListener('click', event => { toggleAutoSafari() });
        ...
        pickItemsBtn.addEventListener('click', event => { toggleAutoPickItems() });
